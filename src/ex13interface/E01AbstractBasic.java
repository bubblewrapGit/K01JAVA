...
/*
 위 클래스를 상속하면 부모가 가진 추상메서드를 그대로 상속받게 됨으로
 해당 클래스도 abstract로 선언해야 한다. 즉, 이 경우 해당 클래스로도 객체를 생성할 수 없게된다.
 만약 객체를 생성해야한다면 추상메서드를 자식에서 오버라이딩 하면 된다.
 */
class AbsClassChild2 extends AbsClass2{
	@Override
	void absMethod(int number) {
		System.out.println("부모클래스에서 오버라이딩한 메소드가 없을땐 에러 발생함.");
	}
	void newMethod() {
...
//		AbsClass1 absClass1 = new AbsClass1(); -> 추상클래스는 객체를 생성할 수 없다.
		
		// 추상클래스를 상속한 하위클래스는 객체생성이 가능하다.
		AbsClassChild1 absClassChild1 = new AbsClassChild1();
		
		/*
		 추상클래스로 인스턴스 생성은 할 수 없지만,
		 참조변수로 사용하는 것은 가능하다.
		 */
		...
		/*
		 추상클래스내에 선언된 정적멤버메서드는 오버라이딩의 대상이 아님으로 
