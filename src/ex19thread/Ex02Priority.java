	...
	public void run() {
		for(int i = 1; i <= 1000; i++) {
			...
			/*
			 ...
			 sleep()메서드를 사용하게되면 실행중 잠깐식 블럭상태로 전환되어
			 우선순위가 낮은 쓰레드도 가끔 실행될 수 있는 상황이 생기게된다.
			 */
			try {
		...
		/*
		 2단계실행
		 	: 우선순위 부여를 위해 정수를 사용한다.
		 	  우선순위는 1~10사이에서 부여할 수 있고 높을수록 우선순위도 높아지게 된다.
		 	  단, 우선순위가 높다고 무조건 실행되지는 않는다.
		 	  전체적인 확률이 높아지게되어 먼저 실행될 가능성이 높아진다고 생각하면 된다.
		 */
...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째", 1);
		...
		/*
		 3단계실행
		 	: 정수대신 static(정적) 살수를 사용하여 우선순위를 부여할 수도 있다.
		 	  결과는 2단계와 동일하다.
		 */
