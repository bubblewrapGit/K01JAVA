...
class Avengers {
	...
	@Override
	public int hashCode() { // hasCode는 정수형태로 반환해준다.
		//각 멤버변수의 hashCode의 값을 얻어온다. 반환타입은 int형이다.
		int nameHCode = this.name.hashCode(); // 네임이라는 변수가 가진 해쉬코드값을 반환해줄수 있다.
	...
	public boolean equals(Object obj) {
		...
		// 매개변수로 전달됨 Object타입의 객체를 Avengers타입으로 다운캐스팅한다.
		Avengers avengers = (Avengers)obj;
		// 각각의 멤버변수가 가진 값에 대한 비교를 진행한다.
		if(this.name.equals(avengers.name) && this.heroName.equals(avengers.heroName) && this.weapon.equals(avengers.weapon)) {
...
public class QuHashSet {
	...
	public static void main(String[] args) {
		...
		boolean isFind = false; //검색한 정보가 잇는지 확인위한 변수
		Iterator<Avengers> itr = set.iterator();
		while(itr.hasNext()) {
			Avengers avn = itr.next();
			
			// 검색할 이름과 이터레이터를 통해 반환되는 객체의 이름 비교
		...
		if(isFind == false) System.out.println("해당 영웅은 없어요");
