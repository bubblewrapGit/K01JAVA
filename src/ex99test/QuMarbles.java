 ...
 */
class ChildProperty{
	static int totalNumOfBead; // 총 구슬의 갯수
	int eachNumOfBead; // 개인의 구슬의 갯수
	
	public ChildProperty(int bead){
		eachNumOfBead = bead;
		totalNumOfBead += eachNumOfBead;
	}
	
	void showProperty(){
		System.out.println("보유 구슬의 개수 " + eachNumOfBead);
	}

	void obtainBead(ChildProperty user, int bead){
		eachNumOfBead = totalNumOfBead - (user.eachNumOfBead - bead);
		user.eachNumOfBead -= bead; 
	}
