...
class MyClass{
	...
	public boolean equals(Object obj) {
		/*
		 매개변수로 전달된 객체를 Object로 받은 후 MyClass의 인스턴스인지 먼저 판단한다.
		 instanceof연산자는 해당 객체와 상속관계가 있는지를 판단한다.
		 */
		if(obj instanceof MyClass) {
			/*
			 매개변수가 MyClass타입이 맞다면 비교를 위해 MyClass타입으로 형변환(다운캐스팅)한다.
			 */
			MyClass mc = (MyClass)obj;
			// 두객체의 멤버변수의 값을 실제로 비교한다.
			if(mc.data == this.data) {
				System.out.println("MyClass-멤버같음");
				return true; // 값이 일치하면 true반환
			}else {
				System.out.println("MyClass-멤버다름");
				return false;// 값이 다르면 false반환
			}
		}else {
			/*
			 매개변수로 전달된 객체가 MyClass의 인스턴스가 아니라면
			 비교의 대상이 될 수 없음으로 false반환
			 */
			return false;
		}
	}
...
class Point{
	...
	/*
	 객체1,2의 x,y값을 동시에 비교해서 둘 다 동일할 때 같은 객체로 판단한다.
...
public class E04Object3Equals {
	public static void main(String[] args) {
		...
		/*
		 println()은 출력 전 항상 객체의 toString() 메서드를
		 자동으로 호출함으로 아래 출력결과는 동일하다.
		 */
		System.out.println("[두 객체의 toString()메소드 호출]");
		System.out.println("mc1.toString()=>" + mc1.toString());
		System.out.println("mc2.toString()=>" + mc1.toString());
