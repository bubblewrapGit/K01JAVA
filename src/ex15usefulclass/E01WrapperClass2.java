...
public class E01WrapperClass2 {
	public static void main(String[] args) {
		...
		System.out.println("10+strNumber를 숫자로 변경 : " + (10 + Integer.parseInt(strNumber))); // 숫자 + 숫자
		System.out.println("10+strNumber를 숫자로 변경 : " + (10 + Integer.valueOf(strNumber))); // 숫자 + 숫자
		
		/*
		 문자열을 숫자로 변경할때는 반드시 숫자형식만 기술해야 한다.
		 "원"은 숫자가 아님으로 예외가 발생한다.
		 */
		String money = "120원";
...
//		System.out.println(Integer.parseInt(floatString)); // 예외발생
		System.out.println("실수형(floast)형으로 변경 : " + Float.parseFloat(floatString));
		System.out.println("실수형(Double)형으로 변경 : " + Double.parseDouble(floatString));
		
		
		// Character클래스의 주요메서드
		/*
		 codePointAt() : 문자열에서 index에 해당하는 한 문자의 아스키코드 값을 반환한다.
		 */
		...
		/*
		 isLetter() : 문자인지 여부를 판단하는 메서드로 특수기호나 숫자형일때는 false를 반환한다.
		 */
