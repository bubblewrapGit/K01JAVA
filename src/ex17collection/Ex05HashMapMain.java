...
/*
 HashMap<K,V>
 	: Map<T> 인터페이스를 구현한 컬렉션 클래스로
 		...
 		- 키값은 종복될 수 없다. 중복될 경우 기존의 데이터를 덮어쓰기한다.
 		- 키값으로 검색함으로 다른 컬렉션에 비해 속도가 빠르다.
 */
public class Ex05HashMapMain {
	public static void main(String[] args) {
		// Map컬렉션생성. key와 Value는 둘다 String형으로 선언.
		HashMap<String, String> map = new HashMap<String, String>();
		
		/*
		 객체저장
		 	: 객체저장 시 기존에 저장된 동일한 key값이 존재하면
		 	  이전에 저장된 객체가 반환된다. 만약 처음 입력이라면 null값이 반환된다.
		 */
		System.out.println("name이라는 키값으로 저장된 이전의 값: " + map.put("name", "홍길동")); // -> null로 반환됨
		
		int number = 20;
//		map.put("age", number); // [에러발생] value타입이 맞지않아 에러발생됨
		...
		/*
		 4] 중복저장
		 	: 기존에 입력된 Key값 name이 있음으로 "홍길동"이 출력된다.
		...
		/*		 
			5-1] 키값을 알고 있을 때 출력하기
				: get(키값) 메서드를 이용한다.
		 */
		System.out.println("키값을 알때 : " + map.get("name")); //최길동 출력
////////////////////////////////////////////////////////////////////////////////////////////////////////
		/*
		 	5-2] 키값을 모를때 출력하기
		...
		/*
		 8] 전체삭제 : removeAll()은 없음. clear()만 사용가능함.
		 */
		map.clear();
		System.out.println("정체삭제 후 객체 수 : " + map.size());
	}
}
