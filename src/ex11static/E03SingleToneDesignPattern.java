...
/*
 싱글톤 디자인 패턴
 	: 클래스를 설계하는 디자인패턴의 하나로써 하나의 인스턴스
 	  즉, 하나의 메모리를 생성해 이를 공유해서 사용하고자 할때 쓰는 패턴이다.
 작성방법]
 	1. 생성자의 접근지정자를 private으로 정의한다.
 	2. 클래스 외부에서는 생성자를 통해 접근할 수 없음으로
 	3. 클래스 내부에 정적 메서드로 해당 클래스의 객체를 반환하도록 정의한다.
 		=> getIstance()
 	4. 위 방법을 통해 객체를 생성하면 메모리에 딱 하나의 객체만 만들어지게 된다.
 */
...
class SingleTone{
	...
	/*
	 ...
	 정적변수는 미리 메서드영역에 생성되어 사용할 준비를 마치게 된다.
	 */
	private static SingleTone single = new SingleTone();
	/*
	 * 생성자를 private으로 선언하면 클래스 외부에서는 호출할 수 없음으로,
	 * new 키워드를 통해 객체를 생성할 수 없게 된다.
	 */
	private SingleTone() {
		//기본생성자
	}
	...
	/*
	 정적메서드로 선언된 getInstance() 메서드에서 해당 객체를 외부로 반환한다.
	 */
