...
/*
 FruitSalesMain 이전 버전은 각 클래스가 멤버변수의 초기값이 고정되어,
 상태가 다른 객체를 생성할 수 없는 단점이 있었다.
 두번째 버전에서는 다양한 초기값을 부여할 수 있도록 초기화메서드를 도입해본다.
 */
...
class FruitSeller2{
	...
	/*
	 기존에 상수에서 일반 변수로 변경한다.
	 클래스의 멤버상수로 정의하는 경어 선언과 동시에 초기화해야함으로
	 상수는 초기값이 없는 형태로는 선언 자체가 불가능하다.
	 */
	
	/*
	 초기화 메서드를 선언하여 객체 생성 후 다양한 형태의 초기화가 가능하다.
	 apple_price를 상수에서 변수로 변경한 이유는 상수는 단 한번만 초기화됨으로
	 멤버메서드에서는 초기화 할 수 없다.
	 멤버메서드는 개발자가 원할 때 언제든지 호출 할 수 있음으로, 상수의 특성과는 맞지 않기 때문.
	 */
...
public class E06FruitSalesMain2
{
	...
	{
		/*
		 초기화 메서드가 정의되었음으로 상태가 다른 객체를 생성할 수 없다.
		 */
