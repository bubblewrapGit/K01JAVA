...
public class R03Method {
	...
	private static void guguRangeShow(int n1, int n2) {
		// 매개변수로 전달된 n1~n2단까지 출력하는 형태
		for(int dan = n1; dan <= n2; dan++) {
			for(int su = 1; su <= 9; su++) {
				System.out.printf("%2d * %1d = %3d  ", dan, su, (dan*su));
			}
			System.out.println();
		}
	}
	...
	// 메서드 타입3 : 세과목의 평균값을 반환하는 메서드
	private static double sungjukResult() {
	...
	// 메서드 타입4 : 주어진 값까지의 합을 구해서 반환하는 메서드
	private static int numberPlus(int endNum) {
		// 정수의 합은 정수임으로 반환타입은 int로 결정하면 된다.
		int total = 0;
	...
	// 연습문제] 업글 구구
	private static void upgradeGugu() {
		/*
		 일반적인 구구단은 각 단마다 1~9까지 출력된다.
		 하지만 업글 구구단은 2단은 2까지, 5단은 5까지만 출력됨으로
		 j의 반복을 i만큼 제한하면 된다.
		 */
		for(int dan = 2; dan < 10; dan++) {
			for(int su = 1; su <= dan; su++) {
				System.out.printf("%2d * %1d = %2d  ", dan, su, (dan*su));
			}
			System.out.println();
		}
	}
