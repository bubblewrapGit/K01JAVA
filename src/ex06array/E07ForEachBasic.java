...
/*
 foreach문 (개선된 for문)
 	: 배열의 일부가 아니라 전체를 참조해야할때 주로 사용하는 개선된 for문을 말한다.
...
public class E07ForEachBasic
{
	public static void main(String[] args)
	{
		...
		/*
		 ...
		 더 이상 참조할 요소가 없을때 for문이 자동으로 종료된다.
		 */
		sum = 0;
		for(int e : arr) {
			System.out.print(e + " ");
		...
		/*
		 ...
		 해당 for문 내에서만 증가하게 된다.
		 */
		System.out.println("배열arr의 요소값 1 증가시키기");
		for(int b : arr) {
			b++;
			System.out.print(b + " "); // 1증가된 값 출력
		}
		...
		for(int c : arr) {
			// 1증가된 부분이 기존배열에 적용되지 않으므로 기존의 값이 출력
			System.out.print(c + " ");
		}
		...
		/*
		 2차원배열은 1차원배열을 겹쳐놓은 형태이므로
		 우선 하나의 행을 읽어와서 각 요소를 하나씩 참조하여 출력한다.
		 */
