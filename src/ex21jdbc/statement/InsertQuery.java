...
public class InsertQuery {
	...
	public InsertQuery() {
		...
		}catch (ClassNotFoundException e) { // 클래스를 찾을 수 없다.
			// ojdbc6.jar 파일이 없거나 연동되지 않았을때
			System.out.println("Oracle 드라이버 로딩 실패");
			e.printStackTrace();
		}catch (SQLException e) { // DB자체를 찾을 수 없다.
	...
	private void execute() {
		try {
			...
			/*
			 	try 문으로 진입했다면 예외가 발생하더라도 무조건 실행되는 구문으로
			 	자원해지를 위한 close()메서드를 호출한다.
			 */
			close();
		}
	}
