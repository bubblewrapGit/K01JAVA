	...
	void execute() {
		try {
			...
			/*
				쿼리실행시 사용하는 메서드 종류
				executeUpdate()
					: 쿼리문이 insert/update/delete와 같이 기존레코드에 영향을 미치는 경우 사용한다.
					  실행후 영향을 받은 행의 갯수를 int형으로 반환한다.
				
				executeQuery()
					: select 쿼리를 실행할 때 호출하는 메서드로 레코드에 영향을 미치치 않고
					  조회만 하는 경우 사용한다.
					  조회한 레코드를 반환값으로 받아야 함으로 ResultSet객체를 반환타입으로 사용한다.
			 */
