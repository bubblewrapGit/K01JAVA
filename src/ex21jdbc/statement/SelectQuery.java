...
/*
 ResultSet 클래스
 	...
 	결과로 변환된 레코드의 처음부터 마지막까지 next()메서드를 통해 확인 후 반복 추출한다.
 	- getXXX() 계열의 메서드
 	오라클의 자료형이 
 		number타입 : getInt()
...
public class SelectQuery extends ConnectDB{
	...
	void execute() {
		try {
			...
			while(rs.next()){
				// 인덱스값으로 지정할 경우 컬럼 추가/삭제에 추가작업이 요구된다. (비선호)
				String id = rs.getString(1); // id컬럼(컬럼의 인덱스값)
				String pw = rs.getString("pass");
				String name =rs.getString("name");
				
				/*
					오라클의 date타입을 getDate()로 추출하면 0000-00-00 형태로 출력된다.
					이 경우 date형 자료가 됨으로 java.sql.Date클래스의 참조변수로 저장해야 한다.
				 */
