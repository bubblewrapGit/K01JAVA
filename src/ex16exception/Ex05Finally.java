...
/*
 	...
 	: 예외 발생과 상관없이 try문에 진입했을때
 	  반드시 실행해야 할 코드가 있는 경우 기술하는 블럭이다.
 	  즉, try문으로 진입하지 않는다면 finally절은 실행되지 않는다.
 */
public class Ex05Finally {
	/*
	 런타임 예외가 발생되는 메소드. 예외발생시 무시하기 위해 throws하고 있다.
	 하지만 이와 같은 예외는 어디서든 반드시 예외처리해야한다.
	 */
	...
	static void tryCatchFinally() {
		...
		try {
			...
			/*
			 try문 진입후 예외발생 여부에 상관없이 해당 구문은 실행된다.
	...
	public static void main(String[] args){
		/*
		 만약 이 지점에서도 예외던지기를 하면
		 프로그램은 즉시 중지된다. 즉, 반드시 처리해야할 예외가 존재한다.
		 */
		try {
			runtime();
