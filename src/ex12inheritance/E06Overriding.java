...
/*
 오버라이딩(Overriding)
 	...
 	똑같은 형태로 재정의 한다는 것이다.
 	하위클래스에서 오버라이딩한 메서드는 상위클래스의 메서드를 가리게됨으로
...
class BaseSpeaker extends Speaker{
	...
	/*
	@Override : '어노테이션'이라 부르고 오버라이딩된 메서드에 추가하여
	 컴파일러에게 알려주는 역할을 한다.
	 */
	@Override
	public void showState() {
		/*
		 ...
		 특히, 오버라이딩 된 메서드를 호출한때 super가 없으면
		 재귀호출이 됨으로 반드시 기술해야 한다.
		 */
		super.showState();
...
public class E06Overriding {
	public static void main(String[] args) {
		...
		baseSpeaker.showState();/*
			자식 객체로 호출하였음으로 자식 클래스에서 오버라이딩한	showState() 메서드가 호출 된다.
			부모쪽의 메서드는 가려지게 된다.
		*/
		...
		/*
		 ...
		 부모타입으로 자식객체를 참조하면 접근범위가 부모객체까지로 제한된다.
		 즉, setBase()는 자식쪽 멤버메서드임으로 호출할 수 없다. (▼ 아래 오류 설명)
		 */
		Speaker speaker = new BaseSpeaker();
		speaker.setVolumn(30);
//		speaker.setBase(40); //에러
		speaker.showState();
	}
}
