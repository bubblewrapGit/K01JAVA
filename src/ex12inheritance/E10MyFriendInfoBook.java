...
/*
 친구를 표현한 최상위 클래스로 해당 프로그램에서는 Friend 클래스로 객체생성은 하지 않을 것이다.
 */ 
class Friend{
	// 멤버변수
	String name;	// 이름
	...
	/*
	 간략 정보를 출력하는 용도의 멤버메서드로 실행부가 없는 상태로 정의
...
class HighFriend extends Friend{
	...
	/*
	 ...
	 super를 통해 호출하고, 자식에서 확장한 변수를 별도의 print문을 통해 출력한다.
	 */
	@Override
	public void showAllData() {
	...
	/*
	 자식에서 오버라이딩하여 재정의한 메서드로 고딩친구의 간략정보를 출력한다.
	 */
	@Override
	public void showBasicInfo() {
...
public class E10MyFriendInfoBook {
	...
	/*
	 프로그램 전반의 흐름에 대해서만 기술하고 관련 메서드를 호출하는 정도로만 구성한다.
...
//			후위 증가로 변수의 값이 다른 코드에 먼저 적용이 되고 두번째로 증가한다.
	...
	/*
	 ...
	 1. 친구 정보를 추가할 때 High 또는 Univ 객체가 배열에 저장된다.
	 2. 입력시 모든 객체는 Friend로 자동 형변환되어 저장된다.
	 3. 정보 출력시 배열에 입력된 객체수 만큼 반복하면서 각 요소를 통해 멤버메서드를 호출한다.
	 4. 이때 오버라이딩된 메서드를 통해 항상 해당 객체의 정보를 출력할 수 있다.
	    참조변수의 타입에 영향을 받지 않고 항상 오버라이딩된 최하위의 메소드가 호출되기 때문이다.
	 */
	// 친구정보 전체보기
	...
	public void deleteInfo() {
		...
		if(deleteIndex== -1) {
			...
			/*
			 객체배열에서 검색된 요소를 삭제한 후
			 입력된 위치의 바로 뒤 요소를 앞으로 하나씩 이동시킨다.
			 numOfFriends 앞에서 1 차감 됨으로 마지막요소는 무시된다.
			 */
