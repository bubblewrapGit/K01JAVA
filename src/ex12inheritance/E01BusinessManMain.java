...
/*
 상속(Inheritance)
 	: 기존에 정의된 클래서에 메서드와 변수를 추가하여
 	  새로운 클래스를 정의하는 것을 만한다. 
 	  부모클래스를 자식클래스가 상속할때는 extends 키워드를 사용한다.
 */

// 부모클래스(상위클래스) : 사람의 일반적인 형태를 추상화
class Man {
	// 멤버변수는 private으로 정의했음으로 클래스 내부에서만 접근가능
		// private은 절대적으로 해당 class 내부에서만 접근이 가능하다.
			// 접근하려면 public 메서드를 타고 들어가야한다.
	private String name;
...
// 자식클래스(하위클래스) : Man클래스를 상속 받아 회사원을 표현
class BusinessMan extends Man{
	/*
	 상속받은 자식클래스는 부모클래스의 생성자를 호출하여 부모객체가 먼저 생성되도록 해야한다.
	...
	public void tellYourInfo() {
		...
		/*
		 부모쪽의 멤버변수 name은 private으로 선언되었음으로
		 상속받은 자식에서도 접근이 불가능하다.
		 private은 무조건 자신의 클래스 내에서만 접근할 수 있다.
		 */
