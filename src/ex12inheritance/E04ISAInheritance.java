...
class Computer{
	...
	//public Computer(){}
	/*
	 생성자를 개발자가 추가하면 디폴트 생성자를 생성하지 않는다.
	 */
	...
	/*
	 ...
	 외부클래스 혹은 상속받은 클래스에서 접근이 불가능함으로,
	 public으로 선언된 메서드를 통해 간접 호출해야한다.
	 
	 상속관계라면 protected로 선언된 메서드로 호출 할 수 도 있다.
	 */
...
/*
 컴퓨터를 상속한 노트북 컴퓨터
 	: 컴퓨터가 가지고있는 기본기능에 휴대이용 부분을 확장하여 정의한다.
 	  휴대이용을 위해 배터리 멤버변수가 추가된다.
 */
class NotebookComputer extends Computer{
	
	// 자식 클래스에서 확장한 변수(배터리 포함)
	int battary; // 배터리 충전량
	
	/*
	 부모생성자 호출을 위해 사용하는 super()는 생략이 가능하지만
	 앞에 다른 문장이 먼저 오면 오류가 발생됨으로 주의해야한다.
	 여기서는 부모의 인자생성자를 호출하고 있다.
	 */
	public NotebookComputer(String name, int initCharge) {
		/*
		 여기서 아래 super문장을 삭제하면 에러가 발생한다.
		 부모클래스에서 인자생성자를 정의하였음으로 
		 디폴트 생성자는 만들어지지 않기 때문이다.
		 */
		super(name);
		battary = initCharge;
	}
	...
	public void movigCal() {
		...
		// private으로 선언된 멤버메서드임으로 직접 호출할 수 없다.
//		keyboardTouch(); //에러발생
//		calculate(); // 에러발생
		calculateFunc();
...
class TabletNotebook extends NotebookComputer{
	...
	public void write(String penInfo) {
		...
		/*
		 A. compareTo(B)
		 	: 문자열 A와 B를 비교하여 0이 반환되면 간튼 문자열로 판단하고,
		 	  -1이나 1이 반환되면 서로 다른 문자열로 판단하는
		 	  String 클래스에 정의된 메서드.
		 */
