...
/*
 구성관계로 표현하기에 적합한 Has-A관계
 	: X Has a Y => X가 Y를 소유(포함)하다.
 		...
 		Has-A 와 같이 소유의 관계를 상속으로 표현하면
 		클래스간의 강한 연결고리가 형성되어 권총이 없는 경찰을 표현하기가 힘들어짐.
 		따라서 이 경우에는 구성관계로 표현하는것이 적합하다.
 		구성관계란 클래스의 멤버로 또 다른 객체를 사용하는 것을 말한다.
 */
class Gun{
	int bullet; // 총알의 갯수
	public Gun(int bNum) { // 생성자
		bullet = bNum;
	}
	public void shutGun() {
...
class Police{
		...
		/*
		 권총의 보유여부 초기화
		 	: 만약 총알이 없는 상태라면 보유한 권총이 없는것으로
		 	  ...
		 	  참조변수가 null값이란 것은 참조할 객체가 없는것을 의미한다.
		 	  즉, Heap(힙영역)에 생성된 객체가 없는것을 말한다.
		 */
		if(bNum <= 0) {
			gun = null;
		}else {
	...
	public void shut() {
		if(gun == null) {
			...
			// 보유하고 있음으로 Gun클래스의 멤버메서드를 호출하여 실행한다.
			gun.shutGun();
		}
	}
